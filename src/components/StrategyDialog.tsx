...
export const StrategyDialog: React.FC<{
  open: boolean
  ...
  actions?: string[]
  example?: string
}> = ({ open, onOpenChange, number, title, icon: Icon, description, actions, example }) => {
  if (!open) return null
  return (
      ...
      >
        ...
        <div className="flex flex-col space-y-1.5 text-center sm:text-left mb-4">
          <div className="flex items-center gap-2 font-semibold leading-none tracking-tight text-lg">
            {Icon ? <Icon className="h-5 w-5 text-primary" /> : <span className="flex h-6 w-6 items-center justify-center rounded-full bg-primary text-xs text-primary-foreground">{number}</span>}
            {title}
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          {actions && actions.length > 0 && (
            <div className="space-y-2">
              <h4 className="text-sm font-medium">Concrete acties:</h4>
